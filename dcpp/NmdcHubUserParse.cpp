...
void NmdcHub::noteSecureCtmRejected(const string& message) {
    if(supportFlags & SUPPORTS_NO_SECURE_CTM)
        return;
    // Hub quoted our address as ip:portS (PtokaX "недопустимый номер порта").
    bool sawSecurePort = false;
    for(size_t i = 0; i + 1 < message.size(); ++i) {
        ...
        while(j < message.size() && message[j] >= '0' && message[j] <= '9')
            ++j;
        if(j > i + 1 && j < message.size() && (message[j] == 'S' || message[j] == 's')) {
            sawSecurePort = true;
            break;
        }
    }
    if(!sawSecurePort)
        return;
    if(Util::findSubString(message, "порт") == string::npos &&
       Util::findSubString(message, "port") == string::npos)
        return;
    supportFlags |= SUPPORTS_NO_SECURE_CTM;
    PeerConnectLog::nmdcRecv(getHubUrl(), "$ConnectToMe TLS suffix rejected by hub — using plain ports");
}
