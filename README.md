...
* Flexible settings for files hashing (speed of hashing, filters for ignoring files, etc.). The hasher skips a file while another process still has it open for writing (Windows share mode; macOS/Linux open-fd scan), drops the TTH if size or mtime changes mid-pass, and retries shortly after the queue drains.
