...
* Share browser opens the list root automatically so flat file lists (no subfolders) show files immediately; Browse folder / jump-to also fills the right pane for the selected folder (no extra click). Type filter lists **Any** and only kinds present in the open file list (**Adult Video** when that list has videos with adult markers in the file or folder name: xxx, porn, 18+, JAV studio codes — as in Transmission). Own files context menu can Open file (file or folder), Open directory (reveal in Finder), Convert to EPUB (writes EPUB next to the FB2/FBD, then shows it in the file manager), and Delete File for files and directories (disk + share index; same idea as Finished uploads/downloads for files). Own file lists include Flylink-style media attrs (BR/WH/MV/MA + TS) scanned via libmediainfo on share refresh / hash (cached in `MediaInfo.xml`; min size setting `MinMediaInfoSize`, default 1 MB).
