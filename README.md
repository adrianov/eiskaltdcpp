...
* List of active transfers (downloads/uploads), including the queue of users waiting for the slot (user may temporary grant extra slot for them). Failed uploads leave the list when the connection closes; finished downloads (including file lists) do not stick as empty “Connection closed” rows.
