...
* Alias-aware downloads avoid duplicate connections to the same NMDC user across multiple hubs. When `$ConnectToMe` times out on a hub, that hub is skipped for the peer and the next attempt uses another online hub. If every online hub times out without ever getting the peer’s IP or a slot-wait (`$MaxedOut`), the peer is treated as unreachable and removed from all Download Queue sources; otherwise the skip list clears and rotation continues.
