...
* Programs with a graphical user interface on Qt 6 (primary) and GTK+ (optional), plus a daemon which may be controlled from the command line or from a Web UI (connected via JSON-RPC). Qt cold start shows the main window before HashIndex.xml / share cache / queue load, listen sockets, finished-transfer SQLite, emoticons/aspell, and hub autoconnect; a successful `files.xml.bz2` cache load postpones the share tree walk until the next auto-refresh minute.
* Multi-threaded download (download fragments of a single file from several sources at once).
* Alias-aware downloads avoid duplicate connections to the same NMDC user across multiple hubs. When `$ConnectToMe` times out on a hub, that hub is skipped for the peer and the next attempt uses another online hub — including a different NMDC queue source for the same peer (each hub is its own user ID there), preferring hubs that worked before. A source merely waiting to retry no longer blocks its own hub aliases from taking the next turn. If every online hub times out (or connect give-up is reached) without a peer response — slot-wait (`$MaxedOut`) or a live download handshake — the peer is dropped from all Download Queue sources (not kept as a bad source) and their ShareIndex rows are purged. ShareIndex and auto-search will not re-index or re-attach them for the rest of the session; a manual download/readd may put them back. Otherwise the skip list clears and rotation continues.
...
* Idle upload slots are replaced when the same peer reconnects (avoids “upload already active” floods from clients that spam `$ConnectToMe` after File Not Available); duplicate reject and upload-connected lines are rate-limited in the connect log. Outgoing `$ConnectToMe` to an IP that already has an idle upload is skipped. Download retries wait for the CONNECTING timeout, then try another hub (skip list); no separate failure cooldown map. A short CTM latch plus “already CONNECTING” blocks `$RevConnectToMe` / upload-notify spam. Slot-wait still backs off outgoing retries after `$MaxedOut`, but inbound slot-grant connects are accepted (and free the local download slot immediately so other sources can use `DOWNLOAD_SLOTS`). Give-up sticks on the queue item until Force or the give-up window. A peer drop after a granted slot soft-retries immediately. With **Allow multiple upload connections from the same user** (default on), additional sockets are accepted while existing uploads are busy (segmented downloads). Download-queue `$MyNick` matching is hub-scoped so another hub’s CTM cannot steal a download CQI and flip it to upload churn.
* Share browser opens the list root automatically so flat file lists (no subfolders) show files immediately; Browse folder / jump-to also fills the right pane for the selected folder (no extra click). Type filter lists **Any** and only kinds present in the open file list (**Audio & Video** matches audio or video; **Adult Video** when a video’s file or folder name carries adult cues: age tags, porn-site and studio brands, performer names, erotic-cinema classics, explicit act/anatomy words in Latin/Cyrillic/Japanese/Korean, JAV label codes — matched on name and full path with word boundaries, plus a mainstream-title veto list). Own-files context menu: Open file (file or folder), Open directory (reveal in Finder), Convert to EPUB (writes an EPUB beside the FB2/FBD, then shows it in the file manager), Rename Folder (on disk and in the share index; one selected folder), Delete File (files only), Delete Other Copy / Delete Other Copies (N) when chosen files have extra shared copies of the same TTH (keeps those files; not shown if two selected rows are copies of one file), and Delete Whole Directory for nested folders (that folder, or the parent of selected files when the parent is not a top-level share — never a share root or its parent). Copy file name copies the path along with the filename, so the directory can be deleted when it is not wanted; Search, Download Queue, Transfers, and Finished lists follow the same rule. Own file lists include Flylink-style media attrs (BR/WH/MV/MA + TS) scanned via libmediainfo on share refresh / hash (cached in `MediaInfo.xml`; skip empty files; probe audio, video, and still images (jpg/png/webp/heic/…) when libmediainfo can read them, so pictures get Resolution as well).
...
* Preferences → Connection shows green / gray / red status dots beside TCP, UDP, TLS, and DHT ports (reachable / unknown / blocked); “Check incoming ports” updates TCP/TLS dots from the online probe, UDP turns green when search datagrams are received, and DHT uses the DHT firewall check.
...
* Advanced search with the ability to group results (same-TTH files and same name+path directories); black list for search results. Sidebar Search rows use bold type when extra results arrive while that search is out of view; opening the row restores the normal weight. Each hub keeps a minimum search interval (app default / favorite override; hub guard text like “You can do 1 search in N seconds” raises it). Hub searches are queued per hub (manual before auto, run at the earliest allowed time); Stop/Close/Clear cancel that tab’s queued hub searches. Local DuckDB/share-index search runs immediately on its own path. Download / get-list from search recreates peers by CID when they are only known from the share index (or were GC’d from the live map), and shows queue errors in the status bar. Outgoing peer connects honor hub wait text (Ledokol “wait N seconds before connecting to other users,” PtokaX/uHub CTM flood) via per-hub `HubConnectPace`: pause, learn min online time, prefer other hubs meanwhile.
* Lists of downloaded and uploaded files (file-type filter on finished downloads and uploads — Search categories plus **Adult Video**, matched by the share browser’s filename and folder tags); ability to save logs of downloads. The sidebar **Finished downloads** heading turns bold and shows how many files completed since the last visit; opening that list clears the badge. Finished downloads context menu can Convert to EPUB for local FB2/FBD files (writes an `.epub` beside the source and shows it in the file manager).
* Lists of public and favorite hubs. Public hubs lists have multiple sources; favorite hubs are extremely flexible in configuration features. Auto hub reconnect uses per-day exponential backoff (15s, 30s, 1 min, …). Toolbar/menu Reconnect waits 5 seconds for the hub to release the old session, then retries every 5 seconds until login, ban, or the daily attempt limit. Hub redirects (`$ForceMove` / ADC) follow a change of TLS mode (e.g. plain NMDC → `nmdcs://`); redirects to a hub you already use are skipped.
* Lists of favorite users (they will receive extra slot for downloading files, etc.).
* List of active transfers (downloads/uploads), including the queue of users waiting for the slot (user may temporary grant extra slot for them). Qt UI sizes and rates (Search, transfers, finished uploads/downloads, status bar, share, hubs, …) share one formatter (`WulforUtil::formatBytes`): compact whole units; from KB up, values ≥ 10 end with 0 (23→20 KB/s, not 720.00 KB/s). Downloads of the same file are always grouped into one expandable line (all sources/users); uploads of the same file to the same IP group similarly (same peer via multiple hubs/connections). Parent rows show `Nick, ... (N/M)` and summed speed; children show nick/hub/encryption. Status stays on “Downloaded/Uploaded …” across short Connecting/Requesting gaps between segments from the same source (no flash). Download “Connecting” rows appear only while a real CONNECTING attempt is in flight (~50s) and are cleared on timeout or give-up (not while merely waiting to retry). Sequential small files from the same user keep one transfer group row (retargeted in place; removal deferred so the connection line is reused). Upload rows that stay on Connected/Connecting without Starting are hidden after 10s (socket kept until idle timeout/disconnect). “Upload complete” and failed upload rows clear after ~2s if no next file starts on that connection; a finished segment of a partly uploaded file stops showing its last speed and clears after ~10s unless the peer asks for more (peers that finish from other sources no longer leave rows hanging); finished downloads (including file lists) do not stick as empty “Connection closed” rows. On quit, hubs and peer connections close gracefully (uploads get a few seconds to finish, then any leftover is forced) so peers can free slots before restart.
* Flexible settings for downloading files (lists of destination directories, directory for incomplete downloads, limitation of number of simultaneous downloads, compressed transfers, check of check sums, etc.). Queue items whose target already matches a local file (same path, size, and TTH — e.g. finished by another app) are removed automatically. Incomplete files with download progress are preferred over not-started items within the same queue priority; among alternatives in that class, files are ordered with alpha-numeric natural sort (e.g. `file1` before `file11`; elapsed times `7 min 58 s` before `1 h 56 min`). The same natural order is used when sorting Name / File / Path columns in Qt grids (share browser, search, download queue, transfers, finished transfers, hubs, queued users); for values with `/` or `\` separators, fewer directory segments sort first (trailing separators ignored). Search lists open sorted by Count (highest first), then Path and File as natural-order tie-breakers. For videos (mkv, mp4, avi, …), the trailing chunk (default 20 MB, Preferences → Downloads) is fetched first so the container index near EOF is available sooner; then download continues from the start. TTH tree (`tthl`) is requested only for large files (≥ 64 MiB); smaller downloads use the TTH root alone. Background auto-search keeps looking for more download sources by TTH even when some already look online (they may be on hubs but refuse downloads). Resume reconnects recorded queue sources again after a connect give-up (and when search rediscovers the same user).
* Free-space gauge for the download volume (yellow below 10 GiB free, red below 1 GiB).
* Support of IP filter and basic antispam. Private-message sidebar **Mark as Spam** stores message bodies and silently discards identical future PMs. Upload peers that spam the same GET (identical type/path/range, including repeated `tthl`) are disconnected and blocked for a short cooldown.
* Search spy (allows one to see search phrases which send other users, but without identifying users of course).
* Search results include **Online** after Nick: the number of distinct users who have that file and are online (0, 1, 2, …).
* Share index (Qt + DuckDB): indexes downloaded user file lists and hub search hits into normalized files/users/locations tables. On startup, cached `FileLists/` past 1 day expire (ListCache fetch time when known, else file mtime). Each file’s first-seen name/path (and case-folded forms) live on the files row; location rows store NULL for strings that match that canonical pair and only keep overrides when they differ. File-list ingest also stores media fields (bitrate, resolution, video, audio) when present; Search results fill those columns asynchronously by TTH so hub/local hit insertion stays off the DB path. Immediate local search matches unique `share_files` first (canonical rows only), then fills any remaining limit from directories/name overrides, and cancels any prior in-flight DuckDB scan on Search/Stop/Clear. The index also adds online cached-list sources to queued downloads by TTH (skipping a fresh list match when the index already knows the user has that TTH)—on user connect, after a file finishes, after a list ingest, after File Not Available when that peer has nothing left in the in-memory user queue, and when the Download Queue paints a waiting row (`usersByTth` → `matchQueue`)—so an idle download socket can be woken for the next matching queue item without reconnecting. Download Queue **User** / waiting **Status** columns use the index as display source of truth (batch on load; size-matched; online counts via ClientManager); context menus still use attached `QueueItem` sources once match attaches them. Stale index rows are dropped when a peer reports File Not Available. Outdated schemas (flat or pre-canonical) are replaced with an empty current schema on first open so the write worker is not blocked; cached lists re-ingest as peers come online. Torn DuckDB ART indexes (`Failed to delete all rows from index`) erase the on-disk file and reopen empty instead of leaving a poisoned handle marked open.
* ADL search with support for Perl-style regular expressions (using PCRE library).
* Flexible filter (with regular expressions support) in users list, search results, public hubs lists, file lists, etc.. (Use ##&lt;regexp&gt; string and read about Qt QRegularExpression syntax.)
* Full-featured chat (different fonts, nick coloring, parsing of magnet links and other links, emoticons, chat search, chat commands, BBCode support, disable/enable/clear chat, spell check (Aspell is used), keywords highlighting in the chat, separator for unread messages, saving of chat logs (logs that exceed the size limit keep only the last 10000 lines at launch), the ability to display IP addresses and countries of users in the chat (depends on hub settings: some of them hide this data for usual users). Hub chat and nick list stay in a fixed split; column fitting cannot let the list overlap chat. Grid autosize leaves dragged columns unchanged, shrinks a column only when its new longest value is at least 30% shorter, and does not run when rows are re-sorted.
...
* Flexible hashing settings (throughput cap, ignore filters, and so on). The hasher thread stays at idle priority so interactive work wins (macOS background QoS / Darwin BG with throttled disk I/O; Linux `SCHED_IDLE` plus idle I/O class; Windows `THREAD_PRIORITY_IDLE`). It skips a path while another process still has it open for writing (Windows share mode; macOS/Linux open-fd scan), discards the TTH if size or mtime changes mid-pass, and retries shortly after the queue drains.
* While the app is inactive or minimized, the process yields CPU and disk much like Transmission (macOS Darwin BG + throttled I/O; Linux `SCHED_BATCH` + idle I/O; Windows background mode). Normal scheduling returns on foreground activation or user input, and again just before shutdown.
...
* Support of user extensions on Lua (in all supported systems).
