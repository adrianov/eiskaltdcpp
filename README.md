...
* List of active transfers (downloads/uploads), including the queue of users waiting for the slot (user may temporary grant extra slot for them). Downloads of the same file are always grouped into one expandable line (all sources/users); uploads of the same file to the same IP group similarly (same peer via multiple hubs/connections). Parent rows show `Nick, ... (N/M)` and summed speed; children show nick/hub/encryption. Status stays on “Downloaded/Uploaded …” across short Connecting/Requesting gaps between segments from the same source (no flash). Sequential small files from the same user keep one transfer group row (retargeted in place; removal deferred so the connection line is reused). Failed uploads leave the list when the connection closes; finished downloads (including file lists) do not stick as empty “Connection closed” rows. On quit, hubs and peer connections close gracefully (uploads get a few seconds to finish, then any leftover is forced) so peers can free slots before restart.
