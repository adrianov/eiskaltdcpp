...
* Alias-aware downloads avoid duplicate connections to the same NMDC user across multiple hubs. When `$ConnectToMe` times out on a hub, that hub is skipped for the peer and the next attempt uses another online hub; once every hub has timed out, the skip list clears and rotation starts again.
