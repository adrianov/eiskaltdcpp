...
* Share browser opens the list root automatically so flat file lists (no subfolders) show files immediately. Own files context menu can Open file, Open directory, and Delete File for files and directories (disk + share index; same idea as Finished uploads/downloads for files).
