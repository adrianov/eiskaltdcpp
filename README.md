...
* Programs with graphical user interface (UI) on Qt (main) and GTK+ (alternative), plus daemon which may be controlled from command line or from Web UI (connected via JSON-RPC). Qt cold start shows the main window before HashIndex.xml / share cache / queue load, listen sockets, finished-transfer SQLite, emoticons/aspell, and hub autoconnect; a successful `files.xml.bz2` cache load postpones the share tree walk until the next auto-refresh minute.
