...
bool FavoriteUsers::eventFilter(QObject *obj, QEvent *e){
    if (e->type() == QEvent::KeyRelease){
        ...
        if (k_e->key() == Qt::Key_Delete && treeView == reinterpret_cast<QTreeView*>(obj)){
            QModelIndexList indexes = treeView->selectionModel()->selectedRows(0);
            for (const auto &i : indexes)
                handleRemove(reinterpret_cast<FavoriteUserItem*>(i.internalPointer())->cid);
            return true;
        }
    }
...
void FavoriteUsers::updateUser(const QString &_cid, const QString &stat){
    UserPtr user = ClientManager::getInstance()->findUser(CID(_tq(_cid)));
